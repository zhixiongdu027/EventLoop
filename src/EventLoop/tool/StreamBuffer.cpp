...
void StreamBuffer::ensure_append_size_with_memory_operator(size_t len) {
    ...
    auto *new_memory_ = new char[new_capacity];
...
ssize_t StreamBuffer::write(int fd, const void *data, size_t len) noexcept {
    ...
    if (static_cast<size_t>(write_res) >= peek_able()) {
...
ssize_t StreamBuffer::read_some(int fd) noexcept {
    ...
    if (static_cast<size_t >(read_res) > append_able()) {
...
template<>
ExecuteState stream_buffer_peek<BlockData>(StreamBuffer *buffer, size_t *length, BlockData *t) {
