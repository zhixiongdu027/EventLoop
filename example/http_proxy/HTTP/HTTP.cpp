...
static bool have_chunked_end(const char *data, size_t length) {
    static const char *chunked_end_str = "0\r\n\r\n";
    static const size_t chunked_end_length = STRLEN("0\r\n\r\n");
    if (length < chunked_end_length) {
        return false;
    }
    const char *post = data + length - chunked_end_length;
    return memcmp(post, chunked_end_str, chunked_end_length) == 0;
}
...
ExecuteState HTTP::parse(const char *buff, size_t length) {
    ...
    get_url_or_response:
    {
        const char *line;
        size_t line_length;
        HTTP::LINE_STATUS line_status = get_http_line(buff + parsed_index_, length - parsed_index_, &line,
                                                      &line_length);
        if (line_status == LINE_OPEN) {
            return ExecuteProcessing;
        }
        assert(line_length >= STRLEN("\r\n"));

        parsed_index_ += line_length;
        size_t url_or_response_pos;
        size_t url_or_response_len;
        switch (line[0]) {
            case 'H':  // HTTP/1.1  or HEAD
                if (line[1] == 'T' &&
                    response_dump(line, line_length, &url_or_response_pos, &url_or_response_len) == 0) {
                    head_info["STATUS"] = std::move(
                            std::make_pair(line - buff + url_or_response_pos, url_or_response_len));
                    todo_ = TODO_GET_HEAD;
                    break;
                }
            case 'G':   //GET
            case 'P':  //POST
                if (url_dump(line, line_length, &url_or_response_pos, &url_or_response_len) == 0) {
                    head_info["URL"] = std::move(
                            std::make_pair(line - buff + url_or_response_pos, url_or_response_len));
                    todo_ = TODO_GET_HEAD;
                    break;
                }
    ...
    get_body:
    {
        ...
        switch (body_len_type_) {
            ...
            case CHUNKED:
                if (!have_chunked_end(buff, length)) {
                    return ExecuteProcessing;
                }
                else {
                    while (true) {
                        const char *chunked_split = (const char *) memmem(buff + parsed_index_, length - parsed_index_,
                                                                          "\r\n", STRLEN("\r\n"));
                        if (chunked_split == NULL) {
                            return ExecuteError;
                        }
                        long data_length = strtol(buff + parsed_index_, NULL, 16);
                        const char *data_begin = chunked_split + STRLEN("\r\n");
